...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "امتلك حصصاً موثقة في أراضٍ زراعية وفنادق بوحدات SAK المرتبطة بسعر الذهب. ابدأ بمبالغ صغيرة وبشفافية كاملة." },
    ],
  }),
  component: Landing,
});
...
function Hero() {
  return (
    <section className="relative overflow-hidden">
      <img src={heroLand} alt="أراضٍ زراعية ذهبية عند الغروب" width={1920} height={1088} className="absolute inset-0 h-full w-full object-cover" />
      ...
      <div className="relative mx-auto flex max-w-7xl flex-col items-start px-4 pt-28 pb-24 lg:px-8 lg:pt-40 lg:pb-36">
        ...
        <p className="mt-6 max-w-xl text-lg leading-relaxed text-foreground/80">
          كل وحدة SAK تعادل 0.1 جرام ذهب وتمثل حصة موثقة في أرضٍ أو أصل حقيقي. استثمر بمبالغ صغيرة، تابع محفظتك لحظياً، وبِع حصتك
          بعد الاستحقاق.
        </p>
        <Link
          to="/auth"
...
const steps = [
  { icon: ShieldCheck, title: "سجّل ووثّق هويتك", body: "أنشئ حسابك وأكمل التحقق من الهوية (KYC) خلال دقائق." },
  { icon: Coins, title: "أودع وحوّل إلى SAK", body: "يُحوَّل إيداعك فوراً إلى وحدات SAK بسعر الذهب لحظة الاعتماد." },
  { icon: TrendingUp, title: "استثمر وتابع أرباحك", body: "اختر أصلك، اشترِ حصتك، وتابع قيمة محفظتك لحظياً حتى الاستحقاق." },
] as const;
    ...
    <section className="bg-navy-soft py-20">
      <div className="mx-auto max-w-7xl px-4 lg:px-8">
        <div className="flex items-end justify-between">
          ...
          <Link to="/projects" className="mb-8 shrink-0 text-sm font-semibold text-gold hover:underline">
            عرض الكل ←
          </Link>
        </div>
...
export function LandCard({ land }: { land: { id: string; title_ar: string; country: string; city: string; asset_type: string; expected_roi: number; maturity_months: number; total_sak_inventory: number; available_sak: number; status: string; cover_image_url: string | null } }) {
  const soldPct = Math.round(((Number(land.total_sak_inventory) - Number(land.available_sak)) / Number(land.total_sak_inventory)) * 100);
  return (
    <Link to="/assets/$landId" params={{ landId: land.id }} className="card-luxe group block overflow-hidden !p-0 transition-transform hover:-translate-y-1">
      <div className="relative h-48 overflow-hidden">
        <img src={landImage(land.cover_image_url)} alt={land.title_ar} loading="lazy" width={1024} height={683} className="h-full w-full object-cover transition-transform duration-500 group-hover:scale-105" />
      ...
      <div className="p-5">
        ...
        <div className="mt-4">
          ...
          <div className="h-1.5 overflow-hidden rounded-full bg-secondary">
            <div className="bg-gold-gradient h-full rounded-full" style={{ width: `${soldPct}%` }} />
          </div>
        </div>
      </div>
    </Link>
  ...
  return (
    <section className="mx-auto max-w-7xl px-4 py-20 lg:px-8">
      <div className="card-luxe gold-ring grid gap-10 p-8 md:grid-cols-2 md:p-12">
        <div>
          <SectionHeading eyebrow="حاسبة الاستثمار" title="كم SAK يعادل استثمارك؟" description="السعر محسوب لحظياً من سعر جرام الذهب العالمي — لا أسعار ثابتة." />
          ...
          />
          <input type="range" min={100} max={50000} step={100} value={usd} onChange={(e) => setUsd(Number(e.target.value))} className="mt-4 w-full accent-[var(--gold)]" />
        </div>
        <div className="flex flex-col justify-center gap-4">
          <CalcRow label="عدد وحدات SAK" value={sak != null ? fmtNum(sak, 2) : "…"} highlight />
          <CalcRow label="ما يعادله من الذهب" value={grams != null ? `${fmtNum(grams, 2)} جرام` : "…"} />
...
function CalcRow({ label, value, highlight }: { label: string; value: string; highlight?: boolean }) {
  return (
    <div className={`flex items-center justify-between rounded-xl px-5 py-4 ${highlight ? "bg-gold/10 gold-ring" : "bg-secondary/60"}`}>
      <span className="text-sm text-muted-foreground">{label}</span>
      <span className={`num text-xl font-bold ${highlight ? "text-gold" : "text-foreground"}`}>{value}</span>
    </div>
  );
}
  ...
  return (
    <section className="border-t border-border/60 bg-navy-soft py-20">
      <div className="mx-auto max-w-3xl px-4 text-center">
        ...
        <p className="mx-auto mt-4 max-w-xl text-muted-foreground">انضم لمستثمرين يمتلكون حصصاً حقيقية موثقة في أراضٍ وأصول مدرّة للدخل.</p>
        <Link
          to="/auth"
          search={{ mode: "register" }}
